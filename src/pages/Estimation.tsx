...
const EstimationPage = () => {
  ...
  const handleCreateEstimation = async () => {
    ...
    try {
      ...
      toast.success(
        scheduledDate
          ? `Estimation créée → Job planifié au ${scheduledDate}${scheduledTime ? " " + scheduledTime : ""} (facture en attente de complétion)`
          : "Estimation créée → Job généré (facture en attente de complétion)"
      );
  ...
  const resetDraft = () => {
    ...
    setWidth(""); setScheduledDate(""); setScheduledTime(""); setBushItems([]); setExtras([]); setDiscounts([]);
    setCutType("trim");
  ...
  useEffect(() => {
    const snapshot: DraftSnapshot = {
      ...
      heightBackLeft, heightBackRight, width, scheduledDate, scheduledTime, extras, discounts, bushItems,
    };
    try {
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground">Si renseignée, le job sera créé avec le statut « planifié » à cette date et heure (par défaut 09:00).</p>
              </div>
            ...
            <div className="rounded-lg border bg-muted/30 p-4 space-y-2">
              ...
              {scheduledDate && (
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium">{scheduledDate}{scheduledTime ? " " + scheduledTime : " 09:00"}</span>
                </div>
              )}
            </div>
            <p className="text-sm text-muted-foreground">
              {scheduledDate
                ? `Un job planifié au ${scheduledDate}${scheduledTime ? " " + scheduledTime : " 09:00"} et une facture brouillon ont été créés automatiquement.`
                : "Un job et une facture brouillon ont été créés automatiquement."
              }
            </p>
