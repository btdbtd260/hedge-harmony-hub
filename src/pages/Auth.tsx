...
const Auth = () => {
  ...
  const validate = () => {
    ...
    if (!eR.success) {
      toast.error(eR.error.issues[0].message);
      return false;
    }
    const pR = passwordSchema.safeParse(password);
    if (!pR.success) {
      toast.error(pR.error.issues[0].message);
      return false;
    }
    return true;
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 bg-gradient-to-br from-emerald-50 via-background to-blue-50 dark:from-background dark:via-background dark:to-background">
      ...
      <Card className="w-full max-w-md shadow-card-lg border-border/60 animate-scale-in">
        <CardHeader className="text-center space-y-3 pb-2">
          <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-primary/10">
            <ScissorsLineDashed className="h-7 w-7 text-primary" />
          </div>
          <div>
            <CardTitle className="text-2xl font-bold tracking-tight">Haie ACF</CardTitle>
            <p className="text-sm text-muted-foreground mt-1">Accès réservé aux membres autorisés</p>
          </div>
        </CardHeader>
