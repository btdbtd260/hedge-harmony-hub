...
import { useCustomers, useJobById, useUpdateJob, useParameters, getClientNameFromList, type DbJob } from "@/hooks/useSupabaseData";
...
export default function JobDetailEditor() {
  ...
  const initialCutType = (job?.cut_type as CutType) || "trim";
  const initialSnap = snap || {};

  const [cutType, setCutType] = useState<CutType>(initialCutType);
  const [useCustomPrice, setUseCustomPrice] = useState(!!initialSnap.custom_price_per_foot);
  const [customCutPrice, setCustomCutPrice] = useState(loadFromSnapshot(initialSnap, "custom_price_per_foot"));

  const twoSides = initialSnap.two_sides || {};
  const [twoSidesLeft, setTwoSidesLeft] = useState(!!twoSides.left);
  const [twoSidesFacade, setTwoSidesFacade] = useState(!!twoSides.facade);
  const [twoSidesRight, setTwoSidesRight] = useState(!!twoSides.right);
  const [twoSidesBack, setTwoSidesBack] = useState(!!twoSides.back);
  const [twoSidesBackLeft, setTwoSidesBackLeft] = useState(!!twoSides.back_left);
  const [twoSidesBackRight, setTwoSidesBackRight] = useState(!!twoSides.back_right);

  const [facadeLength, setFacadeLength] = useState(loadFromSnapshot(initialSnap, "facadeLength", "facade_length"));
  const [leftLength, setLeftLength] = useState(loadFromSnapshot(initialSnap, "leftLength", "left_length"));
  const [rightLength, setRightLength] = useState(loadFromSnapshot(initialSnap, "rightLength", "right_length"));
  const [backLength, setBackLength] = useState(loadFromSnapshot(initialSnap, "backLength", "back_length"));
  const [backLeftLength, setBackLeftLength] = useState(loadFromSnapshot(initialSnap, "backLeftLength", "back_left_length"));
  const [backRightLength, setBackRightLength] = useState(loadFromSnapshot(initialSnap, "backRightLength", "back_right_length"));

  const initHeight = initialSnap.height_mode || "global";
  const [heightMode, setHeightMode] = useState<HeightMode>(initHeight);
  const [heightGlobal, setHeightGlobal] = useState(loadFromSnapshot(initialSnap, "heightGlobal", "height_global"));
  const [heightFacade, setHeightFacade] = useState(loadFromSnapshot(initialSnap, "heightFacade", "height_facade"));
  const [heightLeft, setHeightLeft] = useState(loadFromSnapshot(initialSnap, "heightLeft", "height_left"));
  const [heightRight, setHeightRight] = useState(loadFromSnapshot(initialSnap, "heightRight", "height_right"));
  const [heightBack, setHeightBack] = useState(loadFromSnapshot(initialSnap, "heightBack", "height_back"));
  const [heightBackLeft, setHeightBackLeft] = useState(loadFromSnapshot(initialSnap, "heightBackLeft", "height_back_left"));
  const [heightBackRight, setHeightBackRight] = useState(loadFromSnapshot(initialSnap, "heightBackRight", "height_back_right"));
  const [width, setWidth] = useState(loadFromSnapshot(initialSnap, "width"));

  const initExtras: EstimationExtra[] = initialSnap.extras || [];
  const initDiscounts: EstimationDiscount[] = initialSnap.discounts || [];

  const [extras, setExtras] = useState<EstimationExtra[]>(initExtras);
  const [discounts, setDiscounts] = useState<EstimationDiscount[]>(initDiscounts);
  const [bushItems, setBushItems] = useState<BushItem[]>([]);
  const [scheduledDate, setScheduledDate] = useState(job?.scheduled_date ?? "");
  const [scheduledTime, setScheduledTime] = useState(job?.start_time?.slice(0, 5) ?? "");
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await updateJob.mutateAsync({
        ...
        status: scheduledDate && job.status === "pending" ? "scheduled" : job.status,
  ...
  if (isLoading) {
    return <div className="p-8 text-center text-muted-foreground">Chargement du job…</div>;
  }

  if (!job) {
    return <div className="p-8 text-center text-muted-foreground">Job introuvable.</div>;
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-foreground">Modifier le job</h1>
          <p className="text-muted-foreground">{clientName} — {job.cut_type}</p>
        </div>
      </div>
        ...
        <div className="lg:col-span-5 space-y-4">
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2">
                <Label>Date planifiée <span className="text-muted-foreground font-normal">(optionnelle)</span></Label>
                <Input type="date" value={scheduledDate} onChange={(e) => { setScheduledDate(e.target.value); if (!e.target.value) setScheduledTime(""); }} />
              ...
              <div className="space-y-2">
                ...
                {discounts.map((d, i) => (
                  <div key={d.id} className="flex gap-2 items-center">
                    <Input placeholder="Raison (optionnel)" value={d.description} onChange={(e) => updateDiscount(d.id, "description", e.target.value)} className="flex-1" />
                    <Select value={d.type} onValueChange={(v) => updateDiscount(d.id, "type", v as DiscountType)}>
                    ...
                    <Input type="number" min={0} step="0.01" placeholder="0" value={d.value || ""} onChange={(e) => updateDiscount(d.id, "value", Number(e.target.value))} className="w-24" />
                    <span className="text-xs text-muted-foreground w-16 text-right">−${(discountAmounts[i] ?? 0).toFixed(2)}</span>
                    <Button variant="ghost" size="icon" onClick={() => removeDiscount(d.id)}><Trash2 className="h-4 w-4 text-destructive" /></Button>
                  </div>
                ))}
              </div>
            </CardContent>
